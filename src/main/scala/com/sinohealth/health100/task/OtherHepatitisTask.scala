    ...
    if (args.length != 2) {
      ...
      if (mode.equals("create")) {
        ss.sql("create table " + targetTable + " as " +
          ...
          "(select vid,items_name_comm,results_discrete,is_special,period,system,classify from lis_test_result where classify != '乙肝' and is_special=1 and clean_status=0) temp " +
